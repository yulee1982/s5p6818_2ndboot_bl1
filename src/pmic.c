...
//#define NANOPI_PMIC_INIT  //定义于Config.mak 中 CFLAGS += -D$(BOARD)_PMIC_INIT
//#define NANOPC_T3_PMIC_INIT 

#ifdef NANOPC_T3_PMIC_INIT
...
#ifdef NANOPC_T3_PMIC_INIT
static void PMIC_NanoPC_T3(void)
{
	U8 data;
        //U32 temp;
        //U8 temp1;
	...
	I2C_Read(I2C_ADDR_AXP228, 0x80, &data, 1);  //reg=0x21 data=0x4d
	data = (data & 0x1F) | DCDC_SYS | DCDC_DDR;
	/*
	printf("i2c_read_return=0x%08X\r\n", temp);
	printf("i2c_read=0x%04X\r\n", data);
	temp1 = 0;
	while(1)
	{
	    if(temp1 != 0xff)
	    {
	    printf("I2C_reg=0x%04X\r\n", temp1);
	    temp = I2C_Read(I2C_ADDR_AXP228, temp1, &data, 1);
	    temp1++;
	    printf("i2c_read_return=0x%08X, i2c_read_data=0x%04X\r\n", temp, data);
	    }
	}
	*/
	...
	//------------------
	//I2C_Read(I2C_ADDR_AXP228, AXP228_REG_DC4VOL, &data, 1);
	//printf("DC4VOL=0x%08X;", data);
	//I2C_Read(I2C_ADDR_AXP228, AXP228_REG_DC5VOL, &data, 1);
	//printf("  DC5VOL=0x%08X\r\n", data);
	//---------------
	//while(1);
}
#endif // NANOPC_T3
